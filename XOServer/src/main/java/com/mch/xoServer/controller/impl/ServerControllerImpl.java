...
public class ServerControllerImpl implements ServerController {
  ...
  @Autowired
  public ServerControllerImpl(GameDataSource dataSource) {
  ...
  private void appendEventLog(String str) {
  ...
  private class ClientHandlerThread extends Thread {
    ...
    // constructor
    // socket c
    ClientHandlerThread(Socket socket) throws XOException {
      this.socket = socket;
      ...
      try {
        ...
        if (clientThreads.get(userName) != null) {
          // reply and abort
          sOutput.writeObject(new TransferData<Boolean>(TransferType.LOGIN,
              false, null, userName));
    ...
    public void run() {
      ...
      while (keepGoing) {
        ...
        case INVITE:
          ...
          writeMsg(new TransferData<Character>(TransferType.INVITE, 'X',
              this.enemyName, null), clientThreads.get(this.userName));
          writeMsg(new TransferData<Character>(TransferType.INVITE, 'O',
              this.userName, null), clientThreads.get(this.enemyName));
          break;
        case SAVE:
          dataSource.saveGame(this.game);
          break;
        case LOAD:
          Game loadGame = dataSource.loadGame(this.userName, this.enemyName);
          char[][] response = null;
          if (loadGame != null) {
            this.game = loadGame;
            response = this.game.getTable();
          }
          writeMsg(new TransferData<char[][]>(TransferType.LOAD, response,
              game.getPlayer1(), game.getPlayer2()), clientThreads.get(this.userName));
          writeMsg(new TransferData<char[][]>(TransferType.LOAD, response,
              game.getPlayer1(), game.getPlayer2()), clientThreads.get(this.enemyName));
        case MARK:
          if (cm.getTo() != null) {
            ...
            if (won) {
              appendMessageLog(userName + " won the game: " + userName
                  + " vs. " + enemyName);
              // view.appendEvent(clientThreads.get(from).socket.toString());
              // view.appendEvent(clientThreads.get(to).socket.toString());
              writeMsg(new TransferData<Boolean>(TransferType.WIN, true, null,
                  cm.getFrom()), clientThreads.get(from));
              writeMsg(new TransferData<Boolean>(TransferType.WIN, false, null,
                  cm.getTo()), clientThreads.get(to));
            }
          }
          break;
        case LOGOUT:
        ...
        case WHOISIN:
          ...
          for (ClientHandlerThread ct : clientThreads.values())
            if (ct.enemyName == null && !ct.userName.equals(this.userName))
              userList.add(ct.userName + " -- since " + ct.date);
          writeMsg(new TransferData<List<String>>(TransferType.WHOISIN,
              userList, null, null), this);
          break;
        default:
    ...
    private synchronized boolean writeMsg(TransferData<?> msg,
        ClientHandlerThread to) {
    ...
    private synchronized boolean forwardMessage(TransferData<?> cm,
